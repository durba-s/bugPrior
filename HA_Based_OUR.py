...
from tensorflow.keras.layers import Dense, Input, Bidirectional, TimeDistributed
...
print("The following is the first sentence!")
list(map(print, X_sentences[0]))
...
vocabulary = list(wordvec_model.wv.key_to_index.keys())
...
x_train = x[:int(0.85 * sz)]
x_test = x[int(0.85 * sz):]

y_train = y_trans[:int(0.85 * sz)]
y_test = y_trans[int(0.85 * sz):]
